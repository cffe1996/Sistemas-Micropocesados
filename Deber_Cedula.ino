/*
 * Deber Valide la cédula ecuatoriana que se ingresa por comunicación serial
 */
String cedula = "";
...
void loop() {
  if (Serial.available() > 0) {
    ...
    {
      Serial.println("El # de cedula no es valido");
    } else if (comprobletras(cedula) > 0) {
      Serial.println("Cédula no válida, cadena contiene letras");
    }
    else if (tam == 11){
      String digitoR = cedula.substring(0, 2);
      int digito_region = digitoR.toInt(); 
      delay(200); 
